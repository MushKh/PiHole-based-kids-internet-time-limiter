...
*/
	...
	$line = file_get_contents("myserver/Parameters.txt"); 
	
	if(isset($_GET['message'])){
	  $Mins_text = $_GET['message'];
	}
?>
    ...
    <div class="col-lg-3 col-sm-6">
        ...
        <div class="small-box bg-aqua no-user-select" id="total_queries" title="only A + AAAA queries">
            <div class="inner">
                ...
                <form method='POST' action='myserver/WritePar.php'>
					<table>

						<tr>
							<td>Allowed time (min):</td>
							<td><input type='text' id='Mins' value="<?php echo $line ?>" name='get_minutes'></input></td>
							<td><button onclick=''>Save</button></td>
						</tr>
					</table>
				</form>
            
            <a href="" class="small-box-footer" title="">
                <span >-</span> <?php echo $Mins_text ?>
            </a>
        </div>
    </div>
    <!-- ./col -->
    <div class="col-lg-3 col-sm-6">
        ...
        <div class="small-box bg-red no-user-select">
            <div class="inner">
                ...
                <h3 >  
					<?php	
					$directory = "/admin/myserver";
					$date_string = date("d, m, Y");

					$FileNameBase = 'Limiter_log'.$date_string.'.html';
					$FileNameFull = $directory.'/'.$FileNameBase;

					echo '<a href="'.$FileNameFull.'" title=""> Today times </a><br>';					
